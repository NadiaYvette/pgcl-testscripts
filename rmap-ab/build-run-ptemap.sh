...
taskset -c 12-19 ninja -C build qemu-system-x86_64 > /home/nyc/src/pgcl/qemu-pm-build.log 2>&1
rc=$?
if [ "$rc" != 0 ]; then echo "BUILD-FAIL rc=$rc"; tail -25 /home/nyc/src/pgcl/qemu-pm-build.log; exit 1; fi
echo "qemu OK $(date +%T)"
...
echo "=== pte_maps_frame distribution (1=present-dangler / 0=stale-TLB) + writable split ==="
grep -hoE 'writable=[01] pte_maps_frame=[01]' $D/pm-*.log 2>/dev/null | sort | uniq -c
echo "=== samples ==="
grep -hE 'PGCL143tlbscan' $D/pm-*.log 2>/dev/null | head -8
