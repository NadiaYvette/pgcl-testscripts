...
gcc -O2 -static -o "$ST/repro" "$P/userspace/file_reclaim_race_repro_v4.c"
...
runone() {  # $1=tag $2=bzImage
    local tag=$1 bz=$2 L=$D/v4val-$1.log
    ...
    echo "$tag: DONE=$(grep -aoE 'RRABL DONE corruption=[0-9]+' "$L" | tail -1) killinit=$(grep -ac 'kill init' "$L") clean_file_corrupt=$(grep -aoE 'clean_file_corrupt=[0-9]+' "$L" | awk -F= '{s+=$2}END{print s+0}') workers=$(grep -ac 'RRABL worker' "$L")"
}
echo "=== pgcl4 (expect clean_file_corrupt>0) ==="
