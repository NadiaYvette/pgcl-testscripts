#!/bin/bash
# revrun.sh — self-contained reverse-debug of rrcrash-4.bin (replaces the finicky
# rr-revdebug.sh). Launch replay QEMU (frozen gdbstub :1234) + gdb -batch rev-orphan.gdb.
set -u
P=/home/nyc/src/pgcl; D=$P/rmap-ab; A=$D/abl-initramfs; QB=/home/nyc/src/qemu/build
O=$P/kernel-rpm-build/pgcl4-debug
BIN="${1:-$D/rrcrash-4.bin}"; GDBF="${2:-$D/rev-orphan.gdb}"; TMO="${RRTMO:-3000}"
pkill -9 -f 'qemu-system-x86_64.*rr=[r]eplay' 2>/dev/null; sleep 1
...
taskset -c 12-19 "$QB/qemu-system-x86_64" \
...
QPID=$!
for t in $(seq 1 40); do ss -ltn 2>/dev/null | grep -q ':1234' && break; sleep 1; done
echo "qemu pid=$QPID gdbstub :1234 up=$(ss -ltn 2>/dev/null | grep -c ':1234'); starting gdb (vmlinux=$O/vmlinux)"
# run gdb under a PTY (script -c) so `continue`/`reverse-continue` block foreground
# (batch gdb makes them async on the QEMU gdbstub -> "Cannot execute ... running").
# rev-orphan.gdb ends with `quit` so gdb exits and script returns.
timeout "$TMO" script -q -c "gdb -nx -iex 'set debuginfod enabled off' -iex 'set style enabled off' -iex 'set pagination off' -iex 'set confirm off' -x $GDBF $O/vmlinux" /dev/null 2>&1
