...
- **The 2.4/2.5 forward-port.** I forward-ported and extended Hugh's larpage
  ...
  characterisation of Linux's behaviour at that scale. Large-memory 32-bit was not
  itself new — that same NUMA-Q hardware ran 64 GiB under Sequent's DYNIX/ptx, and
  unverified reports of Linux reaching such sizes via a 2:2 address-space split
  circulated but were never substantiated — so the novelty was the open
  characterisation, not the memory ceiling. Two things made a 64 GiB memmap fit in
  32-bit lowmem: the `struct page` size reductions I had landed earlier, already
  paying off before clustering, and page clustering's one `struct page` per cluster.
  Page clustering was chosen there
