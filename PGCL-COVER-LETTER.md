...
The two compose, and the composition is the point. Clustering shrinks per-page
...
incrementally correct — all generic code an arch's clustered boot exercises
...
```
  A.  Core mm foundation
      ...
      - fault / COW / PTE-install clustering (+ pte_cluster.h)
      - fork copy + zap clustering
      - swap-in clustering
      - rmap: one-kernel-page-per-PVMW-step convention
      - THP split + PMD accounting under clustering
  ...
  C.  Per-arch enablement — one self-contained "<arch>: support page
      clustering (PAGE_MMUSHIFT)" patch per arch:
...
per arch: a clustering unit test, a clustering stress test, the kernel
...
1. **Core-mm conventions** — does the "one `struct page` per kernel page + one
   ...
   maintainers would want clustering expressed? This is the crux of the RFC.
...
- **Academic roots.** Clustering descends from Babaoğlu and Joy's paging work
...
- **The 2.4/2.5 forward-port.** I forward-ported and extended Hugh's larpage
  ...
  array enough to fit a 64 GiB memmap in 32-bit lowmem. Large-memory 32-bit x86 was
  not itself novel — that same NUMA-Q hardware ran 64 GiB under Sequent's DYNIX/ptx,
  and I have heard unconfirmed accounts of Linux reaching comparable sizes via a 2:2
  address-space split — so the contribution was the clustering approach, not the
  memory ceiling. Clustering was chosen there
