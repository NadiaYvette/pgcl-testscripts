			 ...
			 const Elf_Internal_Rela *relocs)
{
  ...
  for (rel = relocs; rel < relend; ++rel)
	...
	/* TPREL: direct thread-pointer relative, no special entries needed. */
	case R_PARISC_TPREL21L:
	case R_PARISC_TPREL14R:
	  need_entry = 0;
	  break;

	/* PLABEL32: 32-bit procedure label.  In static executables, this
	   is just a direct function address, no special entries needed.  */
			      ...
			      struct elf_link_hash_entry *eh)
{
  ...
  switch (r_type)
    ...
    /* TPREL relocations: direct TLS offset (local-exec model).
       In a static executable, the thread pointer (cr27) is set up by
       the runtime, and TLS variables are at fixed offsets from it.
       The value is the symbol's offset within the TLS segment.  */
    case R_PARISC_TPREL21L:
    case R_PARISC_TPREL14R:
      {
	/* Subtract the TLS segment base to get the TP-relative offset.  */
	asection *tls_sec = elf_hash_table (info)->tls_sec;
	if (tls_sec != NULL)
	  value -= tls_sec->vma;

	if (r_type == R_PARISC_TPREL21L)
	  value = hppa_field_adjust (value, addend, e_lrsel);
	else
	  value = hppa_field_adjust (value, addend, e_rrsel);
    ...
    /* PLABEL32: 32-bit procedure label (function pointer).
       For static executables, this is just the function's address.  */
    case R_PARISC_PLABEL32:
      bfd_put_32 (input_bfd, value + addend, hit_data);
      return bfd_reloc_ok;
